...
def save_to_file(
    start_time, end_time, focus_time, focus_content, real_content, focus_feel
):
    # 将专注信息保存到本地文件
    file_path = "pomotodo_log.csv"
    ...
    with open(file_path, "a", encoding="utf-8") as file:
        if not file_exists:
            file.write("开始时间,结束时间,专注时间,计划专注内容,实际做了什么,感想\n")
    # 先把新数据存到列表里
    new_data = f"{start_time},{end_time},{focus_time},{focus_content},{real_content},{focus_feel}\n"
    all_lines = []
    # 通过读取文件，将新数据插入到第二行，实现最新数据在最上面
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            all_lines = f.readlines()
    all_lines.insert(1, new_data)
...
def main():
    ...
    save_to_file(
        start_time, end_time, actual_time, focus_content, real_con, focus_feel
    )  # 保存实际专注时间
