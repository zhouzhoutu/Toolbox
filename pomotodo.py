...
def get_focus_feel():
    """提示用户输入专注感想（可选）"""
    content = input("请输入您的专注感想（可选）：")
    return content if content else "无"

def save_to_file(start_time, end_time, focus_time, focus_content,focus_feel):
    """将专注信息保存到本地文件"""
    file_path = "pomotodo_log.csv"
    file_exists = os.path.exists(file_path)
    with open(file_path,"a",encoding='utf-8') as file:
        if not file_exists:
            file.write("开始时间,结束时间,专注时间,专注内容,感想\n")
        # 写入数据行
        file.write(f"{start_time},{end_time},{focus_time},{focus_content},{focus_feel}\n")
...
def main():
    ...
    focus_feel = get_focus_feel()
    save_to_file(start_time, end_time, actual_time, focus_content, focus_feel)  # 保存实际专注时间
